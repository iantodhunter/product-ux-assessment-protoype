...
function LevelCard({ 
  ...
  return (
    <button
      onClick={onClick}
      className={`h-[576px] relative rounded-2xl shrink-0 w-[302px] cursor-pointer ${
        isSelected ? 'bg-[#e3f2fd] border-2 border-[#2196f3]' : 'bg-[#f8fafd] border border-[#e6eaf0]'
      }`}
    ...
    >
      ...
      <div className={`absolute border border-solid inset-0 pointer-events-none rounded-2xl ${
        isSelected ? 'border-[#2196f3]' : 'border-[#e6eaf0]'
      }`} />
    </button>
  );
}

export function StepThree({ onComplete, onBack, selectedLevel }: StepThreeProps) {
  const [currentSelection, setCurrentSelection] = useState<string | null>(selectedLevel);
        ...
        <div className="box-border content-stretch flex flex-row gap-6 items-start justify-center p-0 max-w-7xl">
          {levels.map((level) => (
            <LevelCard
              ...
              isSelected={currentSelection === level.id}
